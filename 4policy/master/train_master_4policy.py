...
for i in range(NUM_AGENTS):
    ...
    POLICY_MAP[f"blue_agent_{i}_trafficControl"]  = f"Agent{i}_trafficControl"
...
for i in range(NUM_AGENTS):
    OBSERVATION_SPACE[f"Agent{i}_master"] = gymnasium.spaces.Dict({'action_mask': gymnasium.spaces.multi_discrete.MultiDiscrete([2,2,2]),'observations':env.observation_space(f'blue_agent_{i}')['observations'], 'id':gymnasium.spaces.discrete.Discrete(1)})
    ACTION_SPACE[f"Agent{i}_master"] = gymnasium.spaces.discrete.Discrete(3)
    ...
    OBSERVATION_SPACE[f"Agent{i}_trafficControl"] = gymnasium.spaces.Dict({'action_mask': env.observation_space(f"blue_agent_{i}")['action_mask'], 'observations':env.observation_space(f"blue_agent_{i}")['obs_trafficControl']})
    ACTION_SPACE[f"Agent{i}_trafficControl"] = env.action_space(f"blue_agent_{i}")
...
algo_config = (
    PPOConfig()
    .framework("torch")
    .debugging(logger_config={"logdir":"logs/train_master_4policy_default", "type":"ray.tune.logger.TBXLogger"})
    .environment(env="CC4")
...
model_dir = "models/train_master_4policy_default"
