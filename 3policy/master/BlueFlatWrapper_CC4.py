...
USE_BLOCK = False # only used in the 4-policy implementation
USE_FILES = True
USE_DECOYS= True
USE_MESSAGES = True
...
class BlueFlatWrapper(BlueFixedActionWrapper):
    ...
    def __init__(self, env: CybORG, *args, **kwargs):
        ...
        for name in self.agents:
            if "blue" in name:
                self.high_actionmask[name] = np.array([1,1], dtype=np.int64)
    ...
    def reset(self, *args, **kwargs) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        for a in self.agents:
            ...
            # start with investigate, but should not matter
        ...
        for agent, o in sorted(observations.items()):
            if "blue" in agent:
                ...
                if o["success"].name != "IN_PROGRESS":
                    self.high_actionmask[agent] = np.array([1,1], dtype=np.int64)
                else:
                    if self.prev_action[agent] == 0:
                        self.high_actionmask[agent] = np.array([1,0], dtype=np.int64)
                    else:
                        self.high_actionmask[agent] = np.array([0,1], dtype=np.int64)
    ...
    def erase_history_if_recovered(self, agent_name, observation, subnet, subnet_hosts):
        if 'success' not in observation: return # is that even possible?
        if observation['success'].name != 'TRUE': return
