...
class BlueFlatWrapper(BlueFixedActionWrapper):
    ...
    def __init__(self, env: CybORG, *args, **kwargs):
        ...
        for name in self.agents:
            ...
            self.trafficControl_policies[name] = HMARLBlueAgent(name= f"Agent{name[-1]}_trafficControl")
    ...
    def reset(self, *args, **kwargs) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        for a in self.agents:
            ...
            self.prev_action[a] = 0
            master = a+"_master"
            obs[master] = {'observations': self.output[a], 'action_mask': self.high_actionmask[a], 'id':1}
            inf[master] = {'action_mask': self.high_actionmask[a]}
    ...
    def step(
        ...
        **kwargs,
        ...
        # build actions from loaded policies according to the master action
        temp_actions = {}
        for agent in actions:
            if actions[agent] == 0:
                self.prev_action[agent[:12]] = 0
                temp_actions[agent[:12]] = self.inv_policies[agent[:12]].get_action(
                ...
                self.prev_action[agent[:12]] = 1
                temp_actions[agent[:12]] = self.rec_policies[agent[:12]].get_action(
                    {'observations':self.obs_recover[agent[:12]],
                     'action_mask':self.recover_mask[agent[:12]]},
                    self.action_space(agent[:12])
                )
            elif actions[agent] == 2:
                self.prev_action[agent[:12]] = 2
                temp_actions[agent[:12]] = self.trafficControl_policies[agent[:12]].get_action(
                    {'observations':self.obs_trafficControl[agent[:12]],
                     'action_mask':self.trafficControl_mask[agent[:12]]},
                    self.action_space(agent[:12])
                )
        ...
        if USE_MESSAGES == True  and USE_DECOYS == True:
            ...
            for agent in list(temp_actions.keys()):
                messages[agent] = self._encode_messages_v2(agent)
        ...
        for agent, o in sorted(observations.items()):
            if "blue" in agent:
                ...
                rew[master] = rewards[agent]
                term[master] = terminated[agent]
                trunc[master] = truncated[agent]
                inf[master] = {'action_mask':self.high_actionmask[agent]}

                if o["success"].name != "IN_PROGRESS":
                    self.high_actionmask[agent] = np.array([1,1,1], dtype=np.int64)
                else:
                    if self.prev_action[agent] == 0:
                        self.high_actionmask[agent] = np.array([1,0,0], dtype=np.int64)
                    elif self.prev_action[agent] == 1:
                        self.high_actionmask[agent] = np.array([0,1,0], dtype=np.int64)
                    elif self.prev_action[agent] == 2:
                        self.high_actionmask[agent] = np.array([0,0,1], dtype=np.int64)
    ...
    def observation_change(self, agent_name: str, observation: dict) -> np.ndarray:
        ...
        if observation['success'].name == 'IN_PROGRESS':
            ...
            self.high_actionmask[agent_name] = np.array([1,1,1], dtype=np.int64)
