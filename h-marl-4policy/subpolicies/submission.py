...
class Submission:
    ...
    for a in range(5):
        ...
        AGENTS[f"blue_agent_{a}_investigate"] = HMARLBlueAgent(name=f"Agent{a}_investigate")
        AGENTS[f"blue_agent_{a}_recover"] = HMARLBlueAgent(name=f"Agent{a}_recover")
        AGENTS[f"blue_agent_{a}_trafficControl"] = HMARLBlueAgent(name=f"Agent{a}_trafficControl")
